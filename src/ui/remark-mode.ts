...
export function createRemarkMode(options: RemarkModeOptions): RemarkModeController {
  ...
  function applyConfigStyles(): void {
    ...
    // Background uses lighter opacity; line styles use deeper/saturated colors for visibility
    ...
    if (config.highlightStyle === 'background') {
      markStyle = Object.entries(bgColors).map(([c, rgba]) =>
      ...
      markStyle = Object.entries(lineColors).map(([c, rgba]) =>
    ...
    } else if (config.highlightStyle === 'wavy') {
      markStyle = Object.entries(lineColors).map(([c, rgba]) =>
    ...
    } else if (config.highlightStyle === 'border') {
      markStyle = Object.entries(lineColors).map(([c, rgba]) =>
    ...
    dynStyle.textContent = markStyle;
  }
  ...
  // Sentence boundary regex for click-to-annotate
  const SENTENCE_END = /[。.?!？！\n]/;
  ...
  function handleClickToAnnotate(sel: Selection, container: HTMLElement): void {
    ...
    for (let i = globalOffset - 1; i >= 0; i--) {
      if (SENTENCE_END.test(fullText[i])) { sentStart = i + 1; break; }
    }
    let sentEnd = fullText.length;
    for (let i = globalOffset; i < fullText.length; i++) {
      if (SENTENCE_END.test(fullText[i])) { sentEnd = i + 1; break; }
    }

    const sentenceText = fullText.slice(sentStart, sentEnd).trim();
  ...
  function showSidebar(): void {
    ...
    el.innerHTML = `
      <div class="remark-sidebar-header">
        ...
        <div class="remark-sidebar-actions">
          ...
          <button class="remark-sidebar-settings" title="${t('remark_settings', 'Settings')}">⚙️</button>
        </div>
      </div>
      <div class="remark-config-panel" style="display:none;">
        <label><input type="checkbox" data-cfg="autoDeleteEmpty" ${config.autoDeleteEmpty ? 'checked' : ''}> ${t('remark_cfg_auto_delete', 'Auto-delete empty remarks')}</label>
        <label><input type="checkbox" data-cfg="closeAfterCopy" ${config.closeAfterCopy ? 'checked' : ''}> ${t('remark_cfg_close_after_copy', 'Close tab after copy')}</label>
        <div class="remark-config-group">
          <span class="remark-config-label">${t('remark_cfg_highlight_style', 'Highlight style')}</span>
          <select data-cfg="highlightStyle">
            <option value="background" ${config.highlightStyle === 'background' ? 'selected' : ''}>${t('remark_cfg_style_bg', 'Background')}</option>
            <option value="underline" ${config.highlightStyle === 'underline' ? 'selected' : ''}>${t('remark_cfg_style_underline', 'Underline')}</option>
            <option value="wavy" ${config.highlightStyle === 'wavy' ? 'selected' : ''}>${t('remark_cfg_style_wavy', 'Wavy')}</option>
            <option value="border" ${config.highlightStyle === 'border' ? 'selected' : ''}>${t('remark_cfg_style_border', 'Border')}</option>
          </select>
        </div>
        <div class="remark-config-group">
          <span class="remark-config-label">${t('remark_cfg_default_color', 'Default color')}</span>
          <select data-cfg="defaultColor">
            <option value="yellow" ${config.defaultColor === 'yellow' ? 'selected' : ''}>🟡</option>
            <option value="green" ${config.defaultColor === 'green' ? 'selected' : ''}>🟢</option>
            <option value="blue" ${config.defaultColor === 'blue' ? 'selected' : ''}>🔵</option>
            <option value="pink" ${config.defaultColor === 'pink' ? 'selected' : ''}>🩷</option>
          </select>
        </div>
        <div class="remark-config-group">
          <span class="remark-config-label">${t('remark_cfg_font_size', 'Font size')}</span>
          <input type="range" data-cfg="fontSize" min="12" max="16" step="1" value="${config.fontSize}">
          <span class="remark-config-value">${config.fontSize}px</span>
        </div>
        <div class="remark-config-actions">
          <button class="remark-config-reset">${t('remark_cfg_reset', 'Reset to defaults')}</button>
          <button class="remark-config-done">${t('remark_cfg_done', 'Done')}</button>
        </div>
      </div>
    ...
    // Settings toggle
    const settingsBtn = el.querySelector<HTMLButtonElement>('.remark-sidebar-settings');
    const configPanel = el.querySelector<HTMLElement>('.remark-config-panel');
    ...
    // Config: checkboxes
    configPanel?.querySelectorAll<HTMLInputElement>('input[type="checkbox"][data-cfg]').forEach(inp => {
      inp.addEventListener('change', () => {
        const key = inp.dataset.cfg as keyof RemarkConfig;
        if (key) {
          (config as Record<string, unknown>)[key] = inp.checked;
          saveConfig();
        }
      });
    });
    ...
    // Config: selects
    configPanel?.querySelectorAll<HTMLSelectElement>('select[data-cfg]').forEach(sel => {
      sel.addEventListener('change', () => {
        const key = sel.dataset.cfg as keyof RemarkConfig;
        if (key) {
          (config as Record<string, unknown>)[key] = sel.value;
          saveConfig();
          applyConfigStyles();
          renderHighlights(); // re-apply highlight style
        }
      });
    });

    // Config: range sliders
    configPanel?.querySelectorAll<HTMLInputElement>('input[type="range"][data-cfg]').forEach(inp => {
    ...
    // Config: reset
    configPanel?.querySelector('.remark-config-reset')?.addEventListener('click', () => {
      resetConfig();
      applyConfigStyles();
      renderHighlights();
      showSidebar();
    ...
    // Done button — just close the config panel
    configPanel?.querySelector('.remark-config-done')?.addEventListener('click', () => {
  ...
  function injectStyles(): void {
    ...
    style.textContent = `
      ...
      .remark-sidebar-settings {
        border: none; background: none; padding: 4px 6px; cursor: pointer; font-size: 14px;
        color: var(--gray-400, #9ca3af); transition: color 0.15s;
      }
      .remark-sidebar-settings:hover { color: var(--gray-700, #374151); }
      .remark-config-panel {
        padding: 8px 12px; border-bottom: 1px solid var(--border-color, #e5e7eb);
        font-size: 12px; color: var(--gray-600, #4b5563);
      }
      .remark-config-panel label {
        display: block; padding: 4px 0; cursor: pointer; user-select: none;
      }
      .remark-config-panel input[type="checkbox"] { margin-right: 6px; vertical-align: middle; }
      .remark-config-group {
        display: flex; align-items: center; gap: 8px; padding: 4px 0;
      }
      .remark-config-label { min-width: 90px; }
      .remark-config-group select {
        border: 1px solid var(--border-color, #e5e7eb); border-radius: 4px;
        padding: 2px 6px; font-size: 12px; background: var(--color-bg-surface, #fff);
      }
      .remark-config-group input[type="range"] {
        flex: 1; height: 4px; accent-color: var(--gray-500, #6b7280);
      }
      .remark-config-value { min-width: 36px; text-align: right; font-variant-numeric: tabular-nums; }
      .remark-config-actions {
        display: flex; gap: 8px; margin-top: 8px; align-items: center;
      }
      .remark-config-reset {
        border: 1px solid var(--border-color, #e5e7eb); border-radius: 4px;
        background: none; padding: 3px 10px; font-size: 11px; cursor: pointer;
        color: var(--gray-500, #6b7280); transition: color 0.15s, border-color 0.15s;
      }
      .remark-config-reset:hover { color: var(--color-danger, #ef4444); border-color: var(--color-danger-border, #fecaca); }
      .remark-config-done {
        border: 1px solid var(--color-primary, #2563eb); border-radius: 4px;
        background: var(--color-primary, #2563eb); padding: 3px 12px; font-size: 11px;
        cursor: pointer; color: #fff; margin-left: auto;
      }
      .remark-config-done:hover { opacity: 0.85; }
      .remark-sidebar-list {
        flex: 1;
        overflow-y: auto;
        padding: 8px;
